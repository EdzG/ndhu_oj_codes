...
class XOR_mux_device {
public:
    XOR_mux_device() : XOR_mux_device(0) {};
    XOR_mux_device(bool val) {
        if(val) this->enable = &t;
        else this->enable = &f;
        m = new Mux4_1(val);
    ...
    friend std::ostream& operator<<(std::ostream& out, XOR_mux_device x) {
        out << x.output()->output();
        return out;
    }
private:
